...
// Input gray image
bool GrayDecoder::SetMat(int num, cv::Mat pic) {
...
bool GrayDecoder::SetCodeFileName(std::string code_file_path, std::string code_file_name) {
...
bool GrayDecoder::AllocateSpace() {
	...
	this->grey_mats_ = new cv::Mat[this->num_digit_ * 2];
	this->bin_mats_ = new cv::Mat[this->num_digit_];
	return true;
}
...
bool GrayDecoder::Grey2Bin() {
    ...
    uchar threshold = 5;
		this->bin_mats_[binIdx].create(cv::Size(this->img_width_, this->img_height_), CV_8UC1);
