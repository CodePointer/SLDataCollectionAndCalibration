...
private:
	...
	std::string code_file_path_;	
  std::string code_file_name_;
  std::string code_file_suffix_;
	int img_height_;				// 图像的行分辨率
	int img_width_;			// 图像的列分辨率
	bool is_vertical_;		// 设定格雷码方向

	cv::Mat * grey_mats_;	// 输入的灰度图
	cv::Mat * bin_mats_;	// 加工后的二值图
	cv::Mat res_mat_;		// 结果
...
public:
	...
	bool SetCodeFileName(std::string code_file_path, std::string code_file_name);
};
